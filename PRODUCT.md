...
这是 v0.12 的历史判断，保留它是为了说明 Midnight Ink 为什么存在。当前规则改为**双主题分工**：Midnight Ink 服务游戏 / 夜景 / 文化图像；Jieni Gold 服务 AI / Harness / 开源 / 产品 / 个人品牌图像。两者都是经过验证的暗色 Editorial palette；没有新的、经验证的使用场景，不得新增第三套暗色 Editorial palette。
...
12. **风格自定义 / 用户主题色**。诱惑很大，但一旦开放，质量会崩。当前 11 套 palette 是经过视觉验证的上限。让用户传一个 `#A3FF21` 进来，AI 配的衍生色大概率难看。除非有"色板生成器"先过一层，否则不开。
    这是一条**产品边界**：宁可让用户从 11 套已验证 palette 里选一个最接近的，也不让用户精确指定。和"不接 OOTD / 梦核"是同一类决策。

...
这是 v0.12 的历史判断，保留它是为了说明 Midnight Ink 为什么存在。当前规则改为**双主题分工**：Midnight Ink 服务游戏 / 夜景 / 文化图像；Jieni Gold 服务稀疏发布封面、电影感 hero 与品牌收束；教程、解释、截图、流程、数据和持续移动阅读使用 Jieni Ivory。两者都是经过验证的暗色 Editorial palette；没有新的、经验证的使用场景，不得新增第三套暗色 Editorial palette。

这是和"决策 1：拆两套视觉系统不做一站式"同一类思路——**少而准 > 多而平庸**。

Jieni Ivory 把已验证的 palette 边界从 11 套推进到 12 套，但它不是"可以随意自定义"的例外：它只在既有 Jieni 品牌家族中承担已验证的阅读与分发工作——教程、截图、流程、数据和持续移动阅读。密集内容以 Ivory 为根，Gold 仍只承担稀疏封面、电影感 hero 和品牌收束；任意新色、任意逐页混色和用户自定义色仍然不被接受。
...
12. **风格自定义 / 用户主题色**。诱惑很大，但一旦开放，质量会崩。当前 12 套 palette 是经过视觉验证的上限。让用户传一个 `#A3FF21` 进来，AI 配的衍生色大概率难看。除非有"色板生成器"先过一层，否则不开。
    这是一条**产品边界**：宁可让用户从 12 套已验证 palette 里选一个最接近的，也不让用户精确指定。Jieni Ivory 是为既有品牌家族中已验证的阅读/分发任务增加的受限路线，不是打开任意定制的先例。和"不接 OOTD / 梦核"是同一类决策。
